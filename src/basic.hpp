...
public:
    ...
    void PruningModeToggle(unsigned it, unsigned max_local_optima, bool force_on = false) {
        if ((!pruning_set_on && it >= toggle_done && max_local_optima - it >= toggle_left) || force_on) {
            pruning_set_on = true;
...
public:
    ...
    SubgraphResult Run(double max_neg_steps, unsigned max_local_optima, bool do_peeling, unsigned dinkelbach_iterations, double epsilon, double mip_time_limit, bool use_binary) {
        ...
        if (use_binary) {
            ...
            return DinkelbachBinary(cep_result, upper_bound, dinkelbach_iterations, epsilon, mip_time_limit);
        } else {
...
public:
    CEP_QPBO_OPT(const string& input, bool reverse_weight, unsigned toggle_done, unsigned toggle_left)
        : CEP(input, reverse_weight, toggle_done, toggle_left) {
            InitializePositiveWeights();
            vertex_lower_bound = 3; // initialized, but not used in RunMIP (explained there).
            vertex_upper_bound = num_vertices(G);
    ...
    pair<SubgraphResult, bool> FindLowerBound(double max_neg_steps, unsigned max_local_optima, bool do_peeling, bool use_probe = false) {
    ...
    SubgraphResult Run(double max_neg_steps, unsigned max_local_optima, bool do_peeling, double step_size, unsigned direct_mip_bound, unsigned dinkelbach_iterations, double epsilon, double mip_time_limit, bool use_binary, bool use_probe, bool skip_mip_init, bool heuristic_after_mip) {
        // Step 1. Result found by CEP as initial solution
        auto [result_lb, optima] = FindLowerBound(max_neg_steps, max_local_optima, do_peeling, use_probe);
        if (optima) return result_lb;
        
        if (use_binary) {
            // Step 2. Find an upper bound for QPBO
            auto upper_bound = FindUpperBound(result_lb, step_size, use_probe, direct_mip_bound, mip_time_limit);
            // Step 3. Refine the solution by Dinkelbach
            return DinkelbachBinary(result_lb, upper_bound, dinkelbach_iterations, epsilon, mip_time_limit, use_probe, direct_mip_bound, max_local_optima, skip_mip_init, heuristic_after_mip);
        } else {
            return Dinkelbach(result_lb, dinkelbach_iterations, mip_time_limit, use_probe, direct_mip_bound, max_local_optima, skip_mip_init, heuristic_after_mip);
        }      
    }
};