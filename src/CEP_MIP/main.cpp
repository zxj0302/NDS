...
int main(int argc, char* argv[]) {
    ...
    unsigned dinkelbach_iterations = (argc >= 8) ? stoi(argv[7]) : 10;
    double epsilon = (argc >= 9) ? stod(argv[8]) : 1e-4;
    double mip_time_limit = (argc >= 10) ? stod(argv[9]) : 300.0;
    bool use_binary = (argc >= 11) ? (string(argv[10]) == "1") : true;
    unsigned num_its = (argc >= 12) ? stoi(argv[11]) : 1;

    CEP_MIP graph(input, reverse_weight);
