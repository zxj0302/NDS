 ...
 * =========================================================
 */
vector<double> compute_positive_edge_sums(const Graph& G) {
...
/*
 * =========================================================
 * Helper function to remove a subgraph from the graph
 * =========================================================
 */
Graph remove_subgraph(const Graph& G, const set<Vertex>& vertices_to_remove) {
    size_t num_nodes = num_vertices(G);
    Graph new_G(num_nodes);
    
    // Copy node properties for nodes not being removed
    for (Vertex v = 0; v < num_nodes; ++v) {
        if (vertices_to_remove.find(v) == vertices_to_remove.end()) {
            new_G[v].has_self_loop = G[v].has_self_loop;
            new_G[v].self_loop_polarity = G[v].self_loop_polarity;
        }
    }
    
    // Copy edges, excluding those involving removed vertices
    for (auto e_it = edges(G); e_it.first != e_it.second; ++e_it.first) {
        Vertex u = source(*e_it.first, G);
        Vertex v = target(*e_it.first, G);
        
        // Skip edges involving removed vertices
        if (vertices_to_remove.find(u) != vertices_to_remove.end() ||
            vertices_to_remove.find(v) != vertices_to_remove.end()) {
            continue;
        }
        
        // Add the edge to the new graph
        add_edge(u, v, EdgeProperty{G[*e_it.first].edge_polarity}, new_G);
    }
...
pair<FibHeap, double> ecc_greedy(Graph& G, unsigned max_neg_count = 100) {
    ...
    vector<double> pos_weights = compute_positive_edge_sums(G);
...
SubgraphResult find_multi_local_optima(const Graph& G_original, unsigned max_neg_count = 100) {
    Graph G = G_original;  // Working copy
    
    double global_max_density = -numeric_limits<double>::infinity();
    vector<Vertex> best_subgraph;
    
    set<Vertex> all_removed_nodes;
    
    while (true) {
        // Compute positive edge sums for current graph
        vector<double> pos_weights = compute_positive_edge_sums(G);
        
        // Find maximum positive edge sum
        double max_pos_weight = *max_element(pos_weights.begin(), pos_weights.end());
        ...
        // Create a copy of G for this iteration (ecc_greedy modifies the graph)
        Graph G_copy = G;
        
        // Find a local densest subgraph
        auto result = ecc_greedy(G_copy, max_neg_count);
        ...
        // Filter out nodes whose positive edge sum is less than current_density
        set<Vertex> nodes_to_filter;
        for (Vertex v = 0; v < num_vertices(G); ++v) {
            if (pos_weights[v] < current_density) {
                nodes_to_filter.insert(v);
            }
        }
        
        // Combine filtered nodes with current subgraph nodes for removal
        set<Vertex> nodes_to_remove = current_subgraph_set;
        nodes_to_remove.insert(nodes_to_filter.begin(), nodes_to_filter.end());
        
        // Keep track of all removed nodes
        all_removed_nodes.insert(nodes_to_remove.begin(), nodes_to_remove.end());
        
        // If we're removing all nodes, we're done
        if (nodes_to_remove.size() >= num_vertices(G)) {
            break;
        }
        
        // Remove the subgraph and filtered nodes from G
        G = remove_subgraph(G, nodes_to_remove);
...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        for (unsigned iteration = 0; iteration < num_its; ++iteration) {
            ...
            auto result = find_multi_local_optima(G, max_neg);
