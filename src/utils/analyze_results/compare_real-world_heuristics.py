...
density_exclude_datasets = set()

runtime_path = 'results/tex/rw_runtime_table_heu.tex'
density_path = 'results/tex/rw_density_table_heu.tex'
...
def compare_algorithms(base_path='./output/real-world'):
    ...
    # Only compare methods that are explicitly listed in method_abbr.
    algorithms = list(method_abbr.keys())
...
def create_latex_tables(
    ...
    def format_value(val, is_time=False):
        if isinstance(val, (int, float)):
            ...
            if is_time:
                ...
                # Density: keep 2 decimal places
                return f"{val:.2f}"
        else:
            return "-"  # For failed or missing results

    def format_density_value(dataset, algo, density_val):
        """Format density values and compute CEP's relative improvement."""
        formatted = format_value(density_val, is_time=False)
        if algo != 'CEP' or not isinstance(density_val, (int, float)):
            return formatted, None

        baseline_vals = []
        for baseline_algo in ('DCSGreedy', 'NEG_DSD'):
            baseline_val = density_df[density_df['Dataset'] == dataset][baseline_algo].values[0]
            if isinstance(baseline_val, (int, float)):
                baseline_vals.append(float(baseline_val))
        ...
        improvement_pct = (float(density_val) - baseline_max) / baseline_max * 100
        return formatted, f"{improvement_pct:+.1f}\\%"
    ...
    for dataset in datasets:
        ...
        for algo in algorithms:
            ...
            density_formatted, _ = format_density_value(dataset, algo, density_val)
            density_vals.append(density_formatted)
    ...
    for i, algo in enumerate(algorithms):
        ...
        display_name = method_abbr.get(algo, algo.replace('_', '\\_'))
        if algo == 'CEP':
            value_row = f"\\multirow{{2}}{{*}}{{{display_name}}}"
            improvement_row = ""
            for dataset in density_datasets:
                density_val = density_df[density_df['Dataset'] == dataset][algo].values[0]
                formatted_value, improvement_text = format_density_value(dataset, algo, density_val)
                value_row += f" & {formatted_value}"
                improvement_row += f" & {improvement_text if improvement_text is not None else '-'}"
            value_row += " \\\\"
            improvement_row += " \\\\"
            density_latex_lines.append(value_row)
            density_latex_lines.append(improvement_row)
        else:
            row = display_name
            for dataset in density_datasets:
